...
export default function DeveloperPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0a0204] via-[#1a0800] to-[#050005] text-white font-body relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 py-12 pt-56 md:pt-64 animate-fade-in-up-orange"> {/* Padding disesuaikan */}
        ...
        <div className="card-3d-orange bg-[#120500]/60 backdrop-blur-xl border border-red-500/30 rounded-3xl p-8 md:p-10 shadow-[0_0_50px_rgba(220,38,38,0.1)] relative overflow-hidden">
          ...
          {!isLoggedIn ? (
            <form onSubmit={handleLogin} className="space-y-4 max-w-sm">
              ...
              <input type="text" placeholder="Username" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-black/30 border border-orange-500/20 text-white focus:outline-none focus:border-red-500 transition-all" required />
              <input type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-black/30 border border-orange-500/20 text-white focus:outline-none focus:border-red-500 transition-all" required />
              <button type="submit" className="w-full py-3 bg-red-600 hover:bg-red-700 rounded-xl font-heading font-bold tracking-wider transition-all text-white shadow-[0_0_15px_rgba(220,38,38,0.3)]">LOGIN</button>
            </form>
          ) : (
            <form onSubmit={handleUploadBanner} className="space-y-4">
              ...
              <input type="url" placeholder="URL Gambar Banner (Gunakan aspect landscape, cth: 1200x400)" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-black/30 border border-orange-500/20 text-white focus:outline-none focus:border-orange-500 transition-all" required />
