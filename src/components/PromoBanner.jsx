...
export default function PromoBanner() {
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        // Mengarah ke API banner Turso kamu
  ...
  // Render satu set item banner (Ditambahkan fallback properti image_url dari Turso)
  const renderBannerSet = (setPrefix) => (
    banners.map((banner, i) => (
      <div key={`${setPrefix}-${banner.id || i}`} className="flex-shrink-0 group cursor-pointer">
        <img 
          ...
          onError={(e) => { e.target.src = 'https://placehold.co/600x160/120500/fff?text=Banner+Error'; }}
        />
      </div>
    ))
  );
  ...
  return (
    <div className="fixed top-16 left-0 right-0 z-40 bg-cyber-dark/90 backdrop-blur-md border-b border-neon-purple/30 shadow-[0_4px_30px_rgba(168,85,247,0.2)] h-36 md:h-48 flex items-center overflow-hidden">
      ...
      {/* PERBAIKAN UTAMA: 
        - Ditambahkan `flex` dan `gap-8` langsung di container track.
        - `w-max` digabungkan dengan class `.animate-marquee` dari CSS global kamu.
      */}
